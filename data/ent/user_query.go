...
func (uq *UserQuery) QueryFavourite() *FavouriteQuery {
...
func (uq *UserQuery) WithFavourite(opts ...func(*FavouriteQuery)) *UserQuery {
	query := (&FavouriteClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withFavourite = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withFavourite; query != nil {
		if err := uq.loadFavourite(ctx, query, nodes,
			func(n *User) { n.Edges.Favourite = []*Favourite{} },
			func(n *User, e *Favourite) { n.Edges.Favourite = append(n.Edges.Favourite, e) }); err != nil {
			return nil, err
		}
	}
...
func (uq *UserQuery) loadEmails(ctx context.Context, query *EmailQuery, nodes []*User, init func(*User), assign func(*User, *Email)) error {
	...
	for _, n := range neighbors {
		fk := n.user_emails
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_emails" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_emails" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (uq *UserQuery) loadFavourite(ctx context.Context, query *FavouriteQuery, nodes []*User, init func(*User), assign func(*User, *Favourite)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uint64]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Favourite(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.FavouriteColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
