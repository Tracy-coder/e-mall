...
type FavouriteQuery struct {
	...
	withProductFavourite *ProductQuery
	withUserFavourite    *UserQuery
...
// QueryProductFavourite chains the current query on the "product_favourite" edge.
func (fq *FavouriteQuery) QueryProductFavourite() *ProductQuery {
...
// QueryUserFavourite chains the current query on the "user_favourite" edge.
func (fq *FavouriteQuery) QueryUserFavourite() *UserQuery {
...
func (fq *FavouriteQuery) FirstID(ctx context.Context) (id uint64, err error) {
	var ids []uint64
...
func (fq *FavouriteQuery) FirstIDX(ctx context.Context) uint64 {
...
func (fq *FavouriteQuery) OnlyID(ctx context.Context) (id uint64, err error) {
	var ids []uint64
...
func (fq *FavouriteQuery) OnlyIDX(ctx context.Context) uint64 {
...
func (fq *FavouriteQuery) IDs(ctx context.Context) (ids []uint64, err error) {
...
func (fq *FavouriteQuery) IDsX(ctx context.Context) []uint64 {
	ids, err := fq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (fq *FavouriteQuery) Clone() *FavouriteQuery {
	...
	return &FavouriteQuery{
		...
		withProductFavourite: fq.withProductFavourite.Clone(),
		withUserFavourite:    fq.withUserFavourite.Clone(),
...
// WithProductFavourite tells the query-builder to eager-load the nodes that are connected to
// the "product_favourite" edge. The optional arguments are used to configure the query builder of the edge.
func (fq *FavouriteQuery) WithProductFavourite(opts ...func(*ProductQuery)) *FavouriteQuery {
	query := (&ProductClient{config: fq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	fq.withProductFavourite = query
	return fq
}

// WithUserFavourite tells the query-builder to eager-load the nodes that are connected to
// the "user_favourite" edge. The optional arguments are used to configure the query builder of the edge.
func (fq *FavouriteQuery) WithUserFavourite(opts ...func(*UserQuery)) *FavouriteQuery {
	query := (&UserClient{config: fq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	fq.withUserFavourite = query
	return fq
}
...
func (fq *FavouriteQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Favourite, error) {
	var (
		...
		_spec       = fq.querySpec()
		loadedTypes = [2]bool{
			fq.withProductFavourite != nil,
			fq.withUserFavourite != nil,
		}
	)
	...
	if query := fq.withProductFavourite; query != nil {
		if err := fq.loadProductFavourite(ctx, query, nodes, nil,
			func(n *Favourite, e *Product) { n.Edges.ProductFavourite = e }); err != nil {
			return nil, err
		}
	}
	if query := fq.withUserFavourite; query != nil {
		if err := fq.loadUserFavourite(ctx, query, nodes, nil,
			func(n *Favourite, e *User) { n.Edges.UserFavourite = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (fq *FavouriteQuery) loadProductFavourite(ctx context.Context, query *ProductQuery, nodes []*Favourite, init func(*Favourite), assign func(*Favourite, *Product)) error {
	ids := make([]uint64, 0, len(nodes))
	nodeids := make(map[uint64][]*Favourite)
	for i := range nodes {
...
func (fq *FavouriteQuery) loadUserFavourite(ctx context.Context, query *UserQuery, nodes []*Favourite, init func(*Favourite), assign func(*Favourite, *User)) error {
	ids := make([]uint64, 0, len(nodes))
	nodeids := make(map[uint64][]*Favourite)
	for i := range nodes {
		fk := nodes[i].UserID
...
func (fq *FavouriteQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(favourite.Table, favourite.Columns, sqlgraph.NewFieldSpec(favourite.FieldID, field.TypeUint64))
	_spec.From = fq.sql
	...
	if fields := fq.ctx.Fields; len(fields) > 0 {
		...
		if fq.withProductFavourite != nil {
			_spec.Node.AddColumnOnce(favourite.FieldProductID)
		}
		if fq.withUserFavourite != nil {
