...
func (oq *OrderQuery) QueryUserOrder() *UserQuery {
...
func (oq *OrderQuery) QueryProductOrder() *ProductQuery {
...
func (oq *OrderQuery) Clone() *OrderQuery {
	...
	return &OrderQuery{
		...
		withUserOrder:    oq.withUserOrder.Clone(),
		withProductOrder: oq.withProductOrder.Clone(),
...
func (oq *OrderQuery) WithUserOrder(opts ...func(*UserQuery)) *OrderQuery {
	query := (&UserClient{config: oq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	oq.withUserOrder = query
	return oq
}
...
func (oq *OrderQuery) WithProductOrder(opts ...func(*ProductQuery)) *OrderQuery {
	query := (&ProductClient{config: oq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	oq.withProductOrder = query
	return oq
}
...
func (oq *OrderQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Order, error) {
	var (
		...
		_spec       = oq.querySpec()
		loadedTypes = [2]bool{
			oq.withUserOrder != nil,
			oq.withProductOrder != nil,
		}
	)
	...
	if query := oq.withUserOrder; query != nil {
		if err := oq.loadUserOrder(ctx, query, nodes, nil,
			func(n *Order, e *User) { n.Edges.UserOrder = e }); err != nil {
			return nil, err
		}
	}
	if query := oq.withProductOrder; query != nil {
		if err := oq.loadProductOrder(ctx, query, nodes, nil,
			func(n *Order, e *Product) { n.Edges.ProductOrder = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (oq *OrderQuery) loadUserOrder(ctx context.Context, query *UserQuery, nodes []*Order, init func(*Order), assign func(*Order, *User)) error {
...
func (oq *OrderQuery) loadProductOrder(ctx context.Context, query *ProductQuery, nodes []*Order, init func(*Order), assign func(*Order, *Product)) error {
	ids := make([]uint64, 0, len(nodes))
	nodeids := make(map[uint64][]*Order)
	for i := range nodes {
...
func (oq *OrderQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := oq.ctx.Fields; len(fields) > 0 {
		...
		if oq.withUserOrder != nil {
			_spec.Node.AddColumnOnce(order.FieldUserID)
		}
		if oq.withProductOrder != nil {
