...
type CartQuery struct {
	...
	withUserCart    *UserQuery
	withProductCart *ProductQuery
...
func (cq *CartQuery) QueryUserCart() *UserQuery {
...
// QueryProductCart chains the current query on the "product_cart" edge.
func (cq *CartQuery) QueryProductCart() *ProductQuery {
...
func (cq *CartQuery) FirstID(ctx context.Context) (id uint64, err error) {
	var ids []uint64
...
func (cq *CartQuery) FirstIDX(ctx context.Context) uint64 {
...
func (cq *CartQuery) OnlyID(ctx context.Context) (id uint64, err error) {
	var ids []uint64
...
func (cq *CartQuery) OnlyIDX(ctx context.Context) uint64 {
...
func (cq *CartQuery) IDs(ctx context.Context) (ids []uint64, err error) {
...
func (cq *CartQuery) IDsX(ctx context.Context) []uint64 {
	ids, err := cq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (cq *CartQuery) Clone() *CartQuery {
	...
	return &CartQuery{
		...
		withUserCart:    cq.withUserCart.Clone(),
		withProductCart: cq.withProductCart.Clone(),
...
func (cq *CartQuery) WithUserCart(opts ...func(*UserQuery)) *CartQuery {
	query := (&UserClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	cq.withUserCart = query
	return cq
}

// WithProductCart tells the query-builder to eager-load the nodes that are connected to
// the "product_cart" edge. The optional arguments are used to configure the query builder of the edge.
func (cq *CartQuery) WithProductCart(opts ...func(*ProductQuery)) *CartQuery {
	query := (&ProductClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	cq.withProductCart = query
	return cq
}
...
func (cq *CartQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Cart, error) {
	...
	if query := cq.withUserCart; query != nil {
		if err := cq.loadUserCart(ctx, query, nodes, nil,
			func(n *Cart, e *User) { n.Edges.UserCart = e }); err != nil {
			return nil, err
		}
	}
	if query := cq.withProductCart; query != nil {
		if err := cq.loadProductCart(ctx, query, nodes, nil,
			func(n *Cart, e *Product) { n.Edges.ProductCart = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CartQuery) loadUserCart(ctx context.Context, query *UserQuery, nodes []*Cart, init func(*Cart), assign func(*Cart, *User)) error {
	ids := make([]uint64, 0, len(nodes))
	nodeids := make(map[uint64][]*Cart)
	for i := range nodes {
		fk := nodes[i].UserID
...
func (cq *CartQuery) loadProductCart(ctx context.Context, query *ProductQuery, nodes []*Cart, init func(*Cart), assign func(*Cart, *Product)) error {
	ids := make([]uint64, 0, len(nodes))
	nodeids := make(map[uint64][]*Cart)
	for i := range nodes {
...
func (cq *CartQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(cart.Table, cart.Columns, sqlgraph.NewFieldSpec(cart.FieldID, field.TypeUint64))
	_spec.From = cq.sql
	...
	if fields := cq.ctx.Fields; len(fields) > 0 {
		...
		if cq.withUserCart != nil {
			_spec.Node.AddColumnOnce(cart.FieldUserID)
		}
		if cq.withProductCart != nil {
